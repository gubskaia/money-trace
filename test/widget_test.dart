...
void main() {
  group('FinanceController', () {
    test('loads seed snapshot and produces advice', () async {
      ...
      await controller.loadForUser('usr-test');
  ...
  group('AuthController', () {
    test('sign in authenticates with valid credentials', () async {
      ...
      expect(controller.displayName, 'Alex Kowalski');
    });

    test('registration persists after onboarding completes', () async {
      final controller = AuthController(
        repository: MemoryAuthRepository(seedDemoUser: false),
      );
      ...
      final completed = await controller.completeOnboarding();

      expect(completed, isTrue);
      expect(controller.stage, AuthStage.authenticated);
      expect(controller.currentUserId, isNotNull);
    });
  });
  ...
  testWidgets('AuthFlowScreen builds the sign in stage without layout errors', (
    ...
    final authController = AuthController(repository: MemoryAuthRepository());
    final settingsController = AppSettingsController(
      repository: MemoryAppSettingsRepository(),
    );
