    ...
    if (expenses > income && income > 0) {
      advice.add(
        FinanceAdvice(
          ...
          message:
              'This month is currently negative by ${AppFormatters.money(expenses - income)}. It may be worth trimming one flexible category.',
          tone: AdviceTone.warning,
        ),
      );
    }
    ...
    if (snapshot.netFlowThisMonth > 0) {
      advice.add(
        FinanceAdvice(
          ...
          message:
              'You currently have about ${AppFormatters.money(snapshot.netFlowThisMonth)} left after expenses. Part of it could be moved into savings automatically.',
          tone: AdviceTone.success,
        ),
      );
    }
