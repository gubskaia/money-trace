...
class _TransactionsScreenState extends State<TransactionsScreen> {
                              ...
                              children: [
                                for (final group in groupedTransactions) ...[
                                  _DateSectionHeader(
                                    ...
                                    currencyCode: _sectionCurrencyCode(
                                      snapshot,
                                      group.transactions,
                                    ),
                                  ),
  ...
  double _sectionTotal(
    ...
    final targetCurrencyCode = _sectionCurrencyCode(snapshot, transactions);
  ...
  String _sectionCurrencyCode(
    FinanceSnapshot snapshot,
    List<FinanceTransaction> transactions,
  ) {
    final currencyCodes = transactions
        .map((transaction) => snapshot.findAccount(transaction.accountId))
        .whereType<FinanceAccount>()
...
class _DateSectionHeader extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final totalText = '${AppFormatters.groupedNumber(total)} $currencyCode';
...
class _ActivityRecordTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final amountText = AppFormatters.moneyDelta(
      transaction.type == TransactionType.income
          ? transaction.amount
          : -transaction.amount,
