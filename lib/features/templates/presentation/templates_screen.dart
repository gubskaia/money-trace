...
class _TemplateGroupCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final monthlyTotal = templates.fold<double>(
      0,
      (sum, template) => sum + snapshot.convertedTemplateMonthlyEstimate(template),
    );
    final textTheme = Theme.of(context).textTheme;
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          Text(
            _formatCurrency(template.amount, template.currencyCode),
...
class _RecurringTemplateComposerSheetState
  ...
  void initState() {
    ...
    _amountController = TextEditingController(
      text: initialTemplate == null
          ? ''
          : GroupedAmountInputFormatter.formatValue(initialTemplate.amount),
    );
  ...
  Future<void> _submit() async {
    ...
    final amount = GroupedAmountInputFormatter.parse(_amountController.text);
