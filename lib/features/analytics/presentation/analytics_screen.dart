...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        final currencyCode =
            currentAccount?.currencyCode ?? snapshot.reportingCurrencyCode;
        final bundle = _buildAnalyticsBundle(
          snapshot,
          selectedAccountId: effectiveAccountId,
          targetCurrencyCode: currencyCode,
          fallbackColor: context.appPalette.primary,
        );
  ...
  _AnalyticsBundle _buildAnalyticsBundle(
    ...
    final currentDayBuckets = _dailyBuckets(
      snapshot,
      currentTransactions,
      period,
  ...
  List<_DayBucket> _dailyBuckets(
    ...
    for (final transaction in transactions) {
      ...
      final convertedAmount = snapshot.convertAmount(
        transaction.amount,
        fromCurrencyCode: transaction.currencyCode,
        toCurrencyCode: targetCurrencyCode,
      );
      if (transaction.type == TransactionType.income) {
        bucket.incomeTotal += convertedAmount;
      } else {
        bucket.expenseTotal += convertedAmount;
      }
    }
  ...
  List<_BreakdownItem> _buildBreakdownItems(
    ...
    for (final transaction in transactions) {
      ...
      totals.update(
        ...
        (current) => current + convertedAmount,
    ...
    final items = totals.entries.map((entry) {
      ...
      return _BreakdownItem(
        ...
        color: category?.tone.color ?? fallbackColor,
        amount: entry.value,
...
Color _analyticsControlSurface(BuildContext context) {
  return context.appPalette.primary.withValues(alpha: 0.06);
}
