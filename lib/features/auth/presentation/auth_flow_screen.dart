...
class _AuthFlowScreenState extends State<AuthFlowScreen> {
                ...
                child: _HelpButton(
                  onTap: () {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          'This is a local prototype flow. We can connect real auth next.',
                        ),
                      ),
                    );
                  },
  ...
  Widget _buildCompletionStep(BuildContext context) {
    ...
    return _OnboardingLayout(
      ...
      isBusy: controller.isSubmitting,
      onPressed: () async {
        final snapshot = widget.financeController.snapshot;
        final primaryAccount = snapshot?.primaryAccount;

        if (primaryAccount != null) {
          final success = await widget.financeController.updateAccount(
            id: primaryAccount.id,
            name: controller.starterAccountName,
            balance: controller.starterBalance,
            currencyCode: controller.preferredCurrencyCode,
            accentColorValue: primaryAccount.accentColorValue,
          );
              ...
              SnackBar(
                content: Text(
                  widget.financeController.errorMessage ??
                      'Unable to personalize your first account.',
                ),
              ),
            );
            return;
